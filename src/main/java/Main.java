...
public class Main {
	...
	AppleStore AppleStore = new AppleStore(setAllObjectValues(JsonObject, user.getCompletePartNumber()));
	...
	if(scan.nextLine().equalsIgnoreCase("yes") == true)
	{
		...
		while (true)
		{
			...
			AppleStore.setAllValues(setAllObjectValues(JsonObject2, user.getCompletePartNumber()));
